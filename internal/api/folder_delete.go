...
func DeleteFolder(router *gin.RouterGroup) {
	router.DELETE("/folder/:uid", func(ctx *gin.Context) {
		...
		if err := DeleteFolderAndContentsRecursive(tx, folderUID, authPayload.UserUID, authPayload.UserID); err != nil {
...
func DeleteFolderAndContentsRecursive(tx *gorm.DB, folderUID, userUID string, userID uint) error {
	// Step 1: Delete all files in the folder
	if err := DeleteAllFilesInFolder(tx, folderUID, userUID, userID); err != nil {
	...
	for _, childFolder := range childFolders {
		if err := DeleteFolderAndContentsRecursive(tx, childFolder.UID, userUID, userID); err != nil {
			return err
		}
	}

	// Step 4: Delete the folder itself
	if err := query.DeleteFolderByUID(tx, folderUID); err != nil {
		return err
	}
...
func DeleteAllFilesInFolder(tx *gorm.DB, folderUID, userUID string, userID uint) error {
	...
	for _, file := range files {
		...
		if len(usersWithFile) > 1 || len(filesWithUser) > 1 {
			...
			if isOwner {
				...
				if err := query.DeleteFilePermission(tx, userID, f_u.FileID); err != nil {
					return err
				}
				...
				if err != nil {
					...
					if err := user_detail_next.TxUpdate(tx, "storage_used", user_detail_next.StorageUsed+uint(file.Size)); err != nil {
			...
			// Delete file permission for the user
			if err := query.DeleteFilePermission(tx, userID, file.ID); err != nil {
				return err
			}
