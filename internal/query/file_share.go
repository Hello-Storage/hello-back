...
func DeleteFileShareState(db *gorm.DB, fileUID string) {
	var fileShareState entity.FileShareState
	// Search for the sharing state by the file UID
	result := db.Unscoped().Where("file_uid = ?", fileUID).First(&fileShareState)
	if result.Error == nil {
		db.Unscoped().Delete(&fileShareState.PublicFile)
		db.Unscoped().Delete(&fileShareState)
	} else {
